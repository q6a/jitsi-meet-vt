...
import { addMessageVideoTranslatorAI } from "../videotranslatorai/action.web"; //videotranslatorai
import {
    ADD_MESSAGE,
    CLOSE_CHAT,
    OPEN_CHAT,
        ...
        case SEND_MESSAGE: {
            ...
            if (conference) {
                ...
                const participantExists =
                    shouldSendPrivateMessageTo && getParticipantById(state, shouldSendPrivateMessageTo);

                if (shouldSendPrivateMessageTo && participantExists) {
                    dispatch(
                        openDialog(ChatPrivacyDialog, {
...
function _addChatMsgListener(conference: IJitsiConference, store: IStore) {
    ...
    conference.on(JitsiConferenceEvents.CONFERENCE_ERROR, (errorType: string, error: Error) => {
 ...
 * @returns {void}
 */
function _onConferenceMessageReceived(
    store: IStore,
    {
        displayName,
        isGuest,
...
export function handleLobbyMessageReceived(message: string, participantId: string) {
    return async (dispatch: IStore["dispatch"], getState: IStore["getState"]) => {
        _handleReceivedMessage(
            { dispatch, getState },
            { participantId, message, privateMessage: false, lobbyChat: true, timestamp: Date.now() }
        );
    };
}
...
function _handleReceivedMessage(
    ...
    const shouldShowNotification =
        userSelectedNotifications?.["notify.chatMessages"] && !hasRead && !isReaction && !timestamp;
 ...
 * @returns {void}
 */
function _persistSentPrivateMessage(
    { dispatch, getState }: IStore,
    recipientID: string,
    message: string,
    isLobbyPrivateMessage = false
) {
    const state = getState();
...
function _shouldSendPrivateMessageTo(state: IReduxState, action: AnyAction) {
    ...
    const lastMessage = navigator.product === "ReactNative" ? messages[0] : messages[messages.length - 1];
    ...
    const recentPrivateMessage =
        navigator.product === "ReactNative"
            ? recentPrivateMessages[0]
            : recentPrivateMessages[recentPrivateMessages.length - 1];
